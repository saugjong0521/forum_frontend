...
import useBringPost from '@/app/hooks/useBringPost';
...
export default function AdminCommentsAllBox() {
    ...
    const getRowClassName = (comment: any) => {
        let baseClasses = 'cursor-pointer transition-colors h-8'; // 테이블 높이설정 가능
    ...
    try {
        // GetPostParams 형태로 객체를 전달해야 함
        const post = await bringPost({ 
            post_id: comment.post_id 
        });
        
        if (post) {
            console.log('게시글 조회 성공:', post);
            // setPost는 bringPost 내부에서 이미 처리됨
        }
    } catch (error) {
                                ...
                                <div className="flex gap-1 justify-center">
                                    ...
                                    {comment.is_active ? (
                                        <button
                                            ...
                                            disabled={isLoading}
                                            className={`px-1 py-0.5 text-xs rounded transition-colors ${isLoading
